...
# load data
data_demographics_ogtt <- data_demographics_ogtt %>% mutate(GAD)
trainData <-
...
n1 <- Sys.time()
res <- measure_models_LF(models =  models[sub],test_data = testData,prediction_f = prediction_LF,outcome = outcome, time = time,horizon_time_v = horizon_time_v,path_to_export_dt = here(), formula_v = formula_v[sub], band = FALSE, affix = "Cox_all")
Sys.time()- n1


res_dt <- extract_performance_measure(path_to_export_dt =  here(),formula_v = formula_v[sub], affix = "Cox_all")
