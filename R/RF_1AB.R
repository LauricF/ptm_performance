...
subs <- length(formula_v)
n1 <- Sys.time()
# for(i in subs){
foreach(i = subs, .packages = c('randomForestSRC', 'pec','timeROC','survival','SurvMetrics','rms','riskRegression','stringr','dplyr','naniar')) %dopar% {
  model <- create_models(model_2_fit = "rfsrc", formula_v = formula_v[i], arg_l = list(x = TRUE, data = trainData))
  res <- measure_models_LF(models =  model,test_data = testData,prediction_f = prediction_LF,outcome = outcome, time = time,horizon_time_v = horizon_time_v,path_to_export_dt = path_to_export_dt, formula_v = formula_v[i], band = FALSE, affix = "rfsrc_1AB")
}
Sys.time()- n1



res_dt <- extract_performance_measure(path_to_export_dt =  here(),formula_v = formula_v[subs], affix = "rfsrc_1AB")

C_dt <- presence_in_formula(res_dt[[2]], c("GRS2","Index60|AUC_glucose|AUC_ceptide"))

dt1 <- C_dt %>% mutate(group_variable  = case_when(
  GRS2 & `Index60|AUC_glucose|AUC_ceptide` ~ "GRS & Metabolic" ,
  GRS2 ~ "GRS",
  `Index60|AUC_glucose|AUC_ceptide` ~ "Metabolic",
  TRUE ~ "None"))

tAUC <- presence_in_formula(res_dt[[1]], c("GRS2","Index60|AUC_glucose|AUC_ceptide"))
